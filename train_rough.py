...
def train(opt):
    ...
    logger.info('num class: %s' %str(opt.num_class))
    ...
    logger.info('Trainable params num : %s' %sum(params_num))
...
def set_logger():  # 로그 환경을 설정해주는 함수
    ...
    fileHandler = logging.handlers.RotatingFileHandler('./log/deep_' + str(logday) + '.log', maxBytes=fileMaxByte,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data', default='train_data', help='path to training dataset')
    parser.add_argument('--valid_data', default='train_data', help='path to validation dataset')
